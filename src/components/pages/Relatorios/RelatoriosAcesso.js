...
function normalizarPizza(data) {
  ...
  return data.map((item) => ({
    ...
    color:
      item.color ||
      (item.label?.toUpperCase().includes("ATRAS")
        ? cores.amarelo
        : item.label?.toUpperCase().includes("FALT")
          ? cores.vermelho
          : cores.verde),
...
export default function RelatoriosAcesso() {
  ...
  const pizza = normalizarPizza(
    resumoQuery.data?.dados_pizza || resumoQuery.data?.pizza
  );
  const linha = normalizarLinha(
    resumoQuery.data?.dados_linha || resumoQuery.data?.linha
  );

  const linhasTabela = useMemo(() => {
    const base = normalizarTabela(
      detalhesQuery.data?.dados ||
      ...
      <div className={styles.surface}>
        ...
        {metricas.percentual_presenca != null && (
          <p className={styles.helper}>
            Frequência geral:{" "}
            <strong>{Number(metricas.percentual_presenca).toFixed(1)}%</strong>
          </p>
        )}
